...
from src.models.user_model import User, Avatar
...
class ClientApi(BaseAPI):
    ...
    async def create(self, user: UserModel):
        await self.set_data(User(**user.dict()))
    ...
    async def upload_avatar(self, request: Request, user_id: UUID, file: UploadFile | None = File()):
        ...
        result = "OK"
        try:
            link = await self.upload_image(fd=await file.read(),
                                           file_name=create_new_filename(user_id.hex, file.filename))
            await self.set_data(Avatar(user_id=user_id, avatar=link))
    ...
    async def list(self):
        return {"detail": await self.get_full_user_data()}
